...
void setup() {
  ...
  WebSocket_init();                             // Настраиваем и запускаем вебсокет
}
        ...
        case 0:                                     // данные с датчиков
          {
            ...
            sensors += ';';
            sensors += serialReadingBuf;
#else
            sensors = serialReadingBuf;

            message_for_esp.temp_street = "";
            byte i = 0;
            while (serialReadingBuf[i] != ';') {
              message_for_esp.temp_street += serialReadingBuf[i];
              ++i;
            }
        ...
        case 1:                                     // настройки
          {
            ...
            byte i = 0;
            for(unsigned int right_iter = 0; right_iter < serialReadingBuf.length(); right_iter++){
              if(serialReadingBuf[right_iter] == ';'){
                mainSettingsMessage[i++] = serialReadingBuf.substring(left_iter, right_iter);
          ...
          break;
        case 2:                                     // график CO2
          graf_CO2 = serialReadingBuf;
          break;
        case 3:                                     // графики температуры
          graf_TEMP = serialReadingBuf;
          break;
        case 4:                                     // графики энергозатрат
          graf_RUB = serialReadingBuf;
